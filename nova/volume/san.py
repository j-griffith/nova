...
class SolidFireSanISCSIDriver(SanISCSIDriver):
    ...
    def _get_sfaccount_by_name(self, sf_account_name, **kwargs):
        sfaccount = None
    ...
    def _get_replication_by_volume_type(self, ctxt, type_id):
    ...
    def _create_volume_pairing(self, source_id, target_id, replication_info):
        ...
        data = self._issue_api_request('StartVolumePairing',
                                       ...
                                       version='6.0')
        result = self._validate_api_response(data)
        pairing_key = result['volumePairingKey']

        # Set the dest to target_mode
    ...
    def _replicate_volume_with_create(self, volume,
                                      source_id, replication_info):
        # Things we expect in replication_info:
        #   {'mvip': 'ip-addr',
        #    'api_port': 'api_port',
        #    'login': 'login-to-mvip',
        #    'password': 'password'}
        ...
        # BOOKMARK
        LOG.debug('Activate pairing between volume %s and %s' %
                  (volume['id'], source_id))
        replication_info['volume_name'] = volume['name'] + '-secondary'
        volume['rep_data'] = replication_info

        # First we create the destination volume on the specified cluster
    ...
    def create_volume(self, volume):
        ...
        if replication_info:
            alt_model = self._replicate_volume_with_create(volume,
                                                           volume_id,
                                                           replication_info)
            ...
            model_update['secondaries'].append(alt_model)
